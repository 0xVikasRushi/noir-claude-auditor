...
Use this during Phase 6 to generate the final audit report.
...
**Audited by:** Noir Circuit Auditor (Claude Code Skill)
...
[List any `.nr` files excluded and why, or "All `.nr` files were included."]
...
how many unconfirmed, overall risk level.]
...
[List any HIGH CONFIDENCE UNCONFIRMED findings with brief description
and why they need human review]

---

## Areas for Further Investigation

[Optional. Patterns that seemed interesting but didn't reach the threshold
for a finding. Do NOT call these vulnerabilities.
...
- Bug class must be from the taxonomy (8 classes)
...
- Do not include speculative risks or best practices as numbered findings
- Do not use hedging language in confirmed findings. Hedging is appropriate
  only in unconfirmed findings.

---

## Skill Benchmark (append to every report)

After the main report, append this section. It is not for the client — it is
for improving the skill over time. Be honest.
...
benchmark:
  ...
  results:
    ...
    unconfirmed: [count]
    dropped: [count]
    classes_triggered: [list of bug class numbers, e.g., [1, 3, 6]]

  false_positives:
    near_misses: [count of findings that almost passed but were correctly dropped]
    descriptions:
      - "[brief description of each near-miss and why it was dropped]"

  skill_gaps:
    rationalizations_caught: [list from SKILL.md table that you caught yourself doing]
    missing_from_taxonomy:
      - "[any pattern you found that isn't covered by the 8 classes]"
    phases_that_struggled:
      - phase: [1-6]
        issue: "[what was hard or unclear about this phase]"

  new_patterns:
    - bug_class: [number or 'NEW']
      pattern: "[describe the code pattern]"
      detection_heuristic: "[how to spot it]"
      false_positive_risk: LOW | MEDIUM | HIGH

  human_feedback:
    # Fill this in after the human reviews the report
    true_positives_confirmed: [count]
    false_positives_reported: [count]
    missed_bugs_human_found: [count]
    notes: "[any feedback from the human reviewer]"
---
```

**Instructions for the benchmark:**
- Be brutally honest. This data improves the skill.
- `dropped` means candidates that failed the false positive filter.
- `near_misses` means findings you almost reported but correctly killed.
  These are valuable — they reveal where the filter is working.
- `missing_from_taxonomy` is the most important field. New patterns
  discovered during audits get added to `bug-taxonomy.md` in future versions.
- `human_feedback` is left blank. The human fills it in after reviewing.
  If they don't, that's fine — the rest of the benchmark is still useful.

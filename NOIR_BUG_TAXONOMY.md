...
`(x - input_i)` equals the product of `(x - output_i)` at a challenge point.
This proves same multiset at one evaluation point — but [5,5,5,5,5] satisfies
this against [1,2,3,4,5] by coincidence. Same polynomial evaluation doesn't mean
same multiset.
...
### What to check
- Poseidon: what are the exact parameters (t, RF, RP, prime)? Do they achieve
  the claimed security level per the Poseidon paper's capacity requirements?
